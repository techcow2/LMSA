...
export function appendMessage(sender, message, files = null, isStreaming = false) {
    ...
    if (sender === 'ai' || sender === 'user') {
        // Check if this is an image message (format: [IMAGE:url])
        const imageMatch = message.match(/^\[IMAGE:(https?:\/\/[^\]]+)\]$/);

        if (imageMatch && sender === 'ai') {
            // This is a generated image message - render it as an image
            const imageUrl = imageMatch[1];
            const prompt = 'Generated image'; // We don't have the original prompt in history, use generic text

            // Create content container for the image
            ...
            img.onclick = () => {
                if (window.openImageModal) {
                    window.openImageModal(imageUrl, prompt);
                }
            };
            ...
            downloadBtn.onclick = () => {
                if (window.downloadGeneratedImage) {
                    window.downloadGeneratedImage(imageUrl, prompt);
                }
            };
            ...
            // Regular text message - process normally
        ...
        if (!isStreaming) {
            ...
            if (sender === 'user') {
                // Check if this is an image generation command (don't allow editing those)
                const isImageCommand = message && message.trim().toLowerCase().startsWith('/image ');

                // Create edit button for user messages (but not for image commands)
                if (!isImageCommand) {
                ...
                // Create TTS (speaker) button for AI messages (always create, visibility controlled by settings)
                const ttsButton = document.createElement('button');
                ttsButton.classList.add('tts-btn', 'flex', 'items-center', 'hover:text-blue-400', 'transition-colors', 'duration-300');
                ttsButton.innerHTML = '<i class="fas fa-volume-up"></i>';
                ttsButton.title = 'Listen to this message';
                ttsButton.dataset.action = 'tts';
                ttsButton.setAttribute('role', 'button');
                ttsButton.setAttribute('aria-label', 'Text to speech');

                // Store reference to the message element for later use
                ttsButton.dataset.messageId = Date.now(); // Simple unique ID
                messageElement.dataset.messageId = ttsButton.dataset.messageId;

                // Check if TTS is enabled and hide if not
                import('./settings-manager.js').then(({ getTTSEnabled }) => {
                    if (!getTTSEnabled()) {
                        ttsButton.style.display = 'none';
                    }
                });

                controlsContainer.appendChild(ttsButton);
