...
/**
 * Initializes the character gallery with performance optimizations
 */
export function initializeCharacterGallery() {
    ...
    // Ensure the gallery container is properly initialized for animations
    ...
    // Preload the character data to avoid delay when opening gallery
    try {
        const savedCharacters = localStorage.getItem('charactersData');
        if (savedCharacters) {
            charactersData = JSON.parse(savedCharacters);
            debugLog('Preloaded characters data:', Object.keys(charactersData).length);
        }
    } catch (error) {
        debugError('Error preloading characters data:', error);
    }

    debugLog('Character gallery initialized with performance optimizations');
}
...
export function showCharacterGallery(characters) {
    ...
    const animateGalleryOpen = () => {
        // Special handling for first-time opening to ensure transitions work properly
...
/**
 * Hides the character gallery with optimized smooth transition effect
 */
export function hideCharacterGallery() {
    ...
    const transitionEndHandler = (e) => {
        // Only proceed if this is the opacity transition ending
    ...
    characterGalleryContainer.addEventListener('transitionend', transitionEndHandler);

    // Fallback timeout in case the transitionend event doesn't fire
    // This can happen in some browsers or if the transition is interrupted
    const fallbackTimeout = setTimeout(() => {
        // Check if the gallery is still visible
        if (characterGalleryContainer.style.display !== 'none') {
            ...
            import('./ui-manager.js').then(module => {
                module.showWelcomeMessage();
                module.ensureWelcomeMessagePosition();
                debugLog('Showing welcome message after fallback timeout');
            });
        }
    }, 300); // Slightly longer than the transition duration as a safety

    debugLog('Character gallery hiding with optimized animation');
}
