    ...
    if (systemPromptInput) {
        ...
        systemPromptInput.addEventListener('change', () => {
            ...
            if (!activeCharacterId || activeCharacterId === '') {
                ...
                console.log('Saved user-created system prompt:', systemPrompt);
            } else {
                ...
                console.log('Character is active, not marking system prompt as user-created');
            }
        ...
        console.log('Loading system prompt - Saved prompt:', savedPrompt);
        console.log('Is user-created:', savedIsUserCreated);
        ...
        if (savedPrompt && savedPrompt.trim() !== '') {
            ...
            console.log('Loaded system prompt:', systemPrompt, 'User-created:', isUserCreatedSystemPrompt);
...
export function setSystemPrompt(prompt, isFromCharacter = false) {
    console.log('Setting system prompt to:', prompt, 'From character:', isFromCharacter);
    ...
    if ((isCharacterPrompt || isFromCharacter) && !hasActiveCharacter) {
        ...
        if (document.readyState !== 'complete') {
            console.log('Setting character prompt during initialization, but no active character ID is set.');
            console.log('This is likely happening during the loadCharacters phase.');
            // Don't show the warning during initialization
        } else {
            console.warn('Setting character prompt but no active character ID is set. This may cause issues.');
        }
    }
    ...
    if (!isCharacterPrompt && hasActiveCharacter && !isFromCharacter) {
        console.warn('Setting non-character prompt while a character is active. This may override character information.');
    }
    ...
    console.log('System prompt saved. User-created:', isUserCreatedSystemPrompt);
    console.log('Current active character ID:', activeCharacterId);
    console.log('Is character prompt:', isCharacterPrompt);
...
export function isSystemPromptSet() {
    ...
    console.log('isSystemPromptSet check - systemPrompt:', systemPrompt);
    console.log('hasActiveCharacter:', hasActiveCharacter, 'activeCharacterId:', activeCharacterId);
    console.log('hasCharacterInfo:', hasCharacterInfo);
    ...
    if (hasActiveCharacter && !hasCharacterInfo) {
        console.log('Active character found but system prompt does not contain character info. Will attempt to restore.');
...
/**
 * Gets the current disable auto-scroll setting
 * @returns {boolean} - The current disable auto-scroll value
 */
