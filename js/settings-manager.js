...
let ttsEnabled = false; // Text-to-speech feature enabled
let ttsPrivacyAccepted = false; // Whether user has accepted TTS privacy notice
let ttsVoice = 'nova'; // Default TTS voice
let imageGenEnabled = false; // Image generation feature enabled
let imageGenPrivacyAccepted = false; // Whether user has accepted image generation privacy notice
...
export function loadSettings() {
    ...
    loadTTSSetting();
    initializeTTSSetting();
    loadImageGenSetting();
    initializeImageGenSetting();
 ...
 */
export function initializeTTSSetting() {
    const ttsCheckbox = document.getElementById('enable-tts');
    if (ttsCheckbox) {
        // Load saved settings
        const savedTTS = localStorage.getItem('ttsEnabled');
        const savedPrivacy = localStorage.getItem('ttsPrivacyAccepted');
        ...
        ttsCheckbox.addEventListener('change', handleTTSToggleChange);
    }
}

/**
 * Handles TTS toggle change (shows privacy modal every time TTS is enabled)
 */
async function handleTTSToggleChange() {
    const ttsCheckbox = document.getElementById('enable-tts');
    if (!ttsCheckbox) return;

    if (ttsCheckbox.checked) {
        // Show privacy modal every time user enables TTS
        const { showTTSPrivacyModal } = await import('./tts-privacy-modal.js');
        const accepted = await showTTSPrivacyModal();
        ...
        if (accepted) {
            ...
            localStorage.setItem('ttsPrivacyAccepted', 'true');
            debugLog('TTS enabled and privacy accepted');
            updateTTSButtonsVisibility();
        } else {
            // User declined
            ttsCheckbox.checked = false;
            ttsEnabled = false;
            localStorage.setItem('ttsEnabled', 'false');
            debugLog('TTS privacy declined');
            updateTTSButtonsVisibility();
        }
    } else {
        // Disabling TTS
        ttsEnabled = false;
        localStorage.setItem('ttsEnabled', 'false');
        debugLog('TTS disabled');
        updateTTSButtonsVisibility();
    }
}

/**
 * Updates the visibility of all TTS buttons in the UI
 */
function updateTTSButtonsVisibility() {
    ...
    debugLog(`Updated ${ttsButtons.length} TTS buttons visibility to ${ttsEnabled ? 'visible' : 'hidden'}`);
}

/**
 * Loads TTS settings from localStorage
 */
export function loadTTSSetting() {
    const savedTTS = localStorage.getItem('ttsEnabled');
    const savedPrivacy = localStorage.getItem('ttsPrivacyAccepted');
    const savedVoice = localStorage.getItem('ttsVoice');
    ...
    debugLog('TTS settings loaded:', { ttsEnabled, ttsPrivacyAccepted, ttsVoice });
}

/**
 * Saves TTS settings to localStorage
 */
export function saveTTSSetting() {
    const ttsCheckbox = document.getElementById('enable-tts');
    if (ttsCheckbox) {
        ttsEnabled = ttsCheckbox.checked;
        localStorage.setItem('ttsEnabled', ttsEnabled);
        debugLog('TTS setting saved:', ttsEnabled);
    }
}

/**
 * Gets TTS enabled status
...
/**
 * Gets TTS privacy acceptance status
 * @returns {boolean} - Whether user has accepted TTS privacy notice
 */
export function getTTSPrivacyAccepted() {
    return ttsPrivacyAccepted;
}

/**
 * Gets TTS voice preference
 * @returns {string} - TTS voice name
 */
...
/**
 * Initializes Image Generation settings
 */
export function initializeImageGenSetting() {
    const imageGenCheckbox = document.getElementById('enable-image-gen');
    if (imageGenCheckbox) {
        // Load saved settings
        const savedImageGen = localStorage.getItem('imageGenEnabled');
        const savedPrivacy = localStorage.getItem('imageGenPrivacyAccepted');

        if (savedImageGen === 'true') {
            imageGenCheckbox.checked = true;
        ...
        if (savedPrivacy === 'true') {
            imageGenPrivacyAccepted = true;
        }

        // Add event listener for toggle changes
        imageGenCheckbox.addEventListener('change', handleImageGenToggleChange);
    }
}

/**
 * Handles Image Generation toggle change (shows privacy modal every time it is enabled)
 */
async function handleImageGenToggleChange() {
    const imageGenCheckbox = document.getElementById('enable-image-gen');
    if (!imageGenCheckbox) return;

    if (imageGenCheckbox.checked) {
        // Show privacy modal every time user enables Image Generation
        const { showImageGenPrivacyModal } = await import('./image-gen-privacy-modal.js');
        const accepted = await showImageGenPrivacyModal();

        if (accepted) {
            // User accepted
            imageGenEnabled = true;
            imageGenPrivacyAccepted = true;
            localStorage.setItem('imageGenEnabled', 'true');
            localStorage.setItem('imageGenPrivacyAccepted', 'true');
            debugLog('Image Generation enabled and privacy accepted');
        } else {
            // User declined
            imageGenCheckbox.checked = false;
            imageGenEnabled = false;
            localStorage.setItem('imageGenEnabled', 'false');
            debugLog('Image Generation privacy declined');
        }
    } else {
        // Disabling Image Generation
        imageGenEnabled = false;
        localStorage.setItem('imageGenEnabled', 'false');
        debugLog('Image Generation disabled');
    }
}

/**
 * Loads Image Generation settings from localStorage
 */
export function loadImageGenSetting() {
    const savedImageGen = localStorage.getItem('imageGenEnabled');
    const savedPrivacy = localStorage.getItem('imageGenPrivacyAccepted');
    ...
    debugLog('Image Generation settings loaded:', { imageGenEnabled, imageGenPrivacyAccepted });
}

/**
 * Saves Image Generation settings to localStorage
 */
export function saveImageGenSetting() {
    const imageGenCheckbox = document.getElementById('enable-image-gen');
    if (imageGenCheckbox) {
        imageGenEnabled = imageGenCheckbox.checked;
        localStorage.setItem('imageGenEnabled', imageGenEnabled);
        debugLog('Image Generation setting saved:', imageGenEnabled);
    }
}

/**
 * Gets Image Generation enabled status
 * @returns {boolean} - Whether Image Generation is enabled
 */
export function getImageGenEnabled() {
    return imageGenEnabled;
}

/**
 * Gets Image Generation privacy acceptance status
 * @returns {boolean} - Whether user has accepted Image Generation privacy notice
 */
export function getImageGenPrivacyAccepted() {
    return imageGenPrivacyAccepted;
}
