        ...
        sortedChats.forEach(([id, chatData]) => {
            try {
                ...
                if (title) {
                    ...
                    // Check if first message is an image command or if there's an image in the chat
                    const firstMessage = messages[0].content || '';
                    const isImageCommand = firstMessage.trim().toLowerCase().startsWith('/image ');
                    const hasImageMessage = messages.some(msg => msg.content && msg.content.startsWith('[IMAGE:'));

                    if (isImageCommand || hasImageMessage) {
                        // For image generation chats, show "Image Generated" as title
                        chatTitle.textContent = 'Image Generated';
                        button.title = 'Image Generated';
                        debugLog('Using "Image Generated" title for image chat');
                    } else {
                        // Fall back to using the first message content, but remove any <think> tags
                        const cleanContent = removeThinkTags(firstMessage);
