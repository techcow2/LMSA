...
export function showSettingsModal() {
    ...
    requestAnimationFrame(() => {
        ...
        // Handle desktop view (show all steps at once)
        if (window.innerWidth >= 1024) {
            ...
            // Show all steps for desktop view
            ...
            // Hide navigation buttons container on desktop
            const navButtons = document.getElementById('settings-navigation-buttons');
            if (navButtons) {
                navButtons.classList.add('hidden');

                // Also hide all button containers to ensure they don't appear
...
export function initializeSettingsModalNavigation() {
    ...
    function showStep(stepName, direction = null) {
        ...
        // For desktop view (large screens), show all steps at once
        if (window.innerWidth >= 1024) {
            // Make sure all steps are visible in desktop view
            Object.values(steps).forEach(step => {
                if (step) {
                    step.classList.remove('hidden');
                    step.classList.add('active');
            ...
            // Ensure navigation buttons are hidden in desktop view
            const navButtons = document.getElementById('settings-navigation-buttons');
            if (navButtons) {
                navButtons.classList.add('hidden');
    ...
    // Initialize desktop view (show all steps at once on large screens)
    if (window.innerWidth >= 1024) {
        showStep('connection');
    }
    ...
    window.addEventListener('resize', () => {
        ...
        if (window.innerWidth >= 1024) {
            ...
            // Mobile/tablet view - show only the first step
            showStep('connection');
...
function initializeSystemPromptOverlay() {
    ...
    function saveChanges() {
        ...
        console.log('Saved system prompt from editor:', editor.value);
        console.log('isUserCreatedSystemPrompt:', localStorage.getItem('isUserCreatedSystemPrompt'));
...
function clearSystemPrompt() {
    ...
    import('./settings-manager.js').then(module => {
        ...
        console.log('System prompt cleared successfully');
    });
